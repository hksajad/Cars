      ...
      {carsData.map((indexcar) => (
        <div className="items_cars flex flex-col flex-wrap ">
          ...
          <div className="img_cars max-[768px]:m-2">
            <Image key={indexcar.id} src={indexcar.image} width={300} height={300} className="w-[250px] h-[250px] rounded-lg m-5 max-[768px]:w-full max-[768px]:m-auto"/>
          </div>
          {/* name cars */}
          <div className="name_cars  flex flex-col">
            <h1 className="text-center font-bold text-[20px]" key={indexcar.id}>{indexcar.name}</h1>
          </div>
          {/* location and prise */}
          <div className="location flex justify-around items-center mt-5 mb-5">
            <h1 className="bg-green-200 rounded-lg px-1 py-1" key={indexcar.id}>{indexcar.price}</h1>
            <h1 key={indexcar.id}>{indexcar.location}</h1>
          </div>
        </div>
      ))}
    </div>
