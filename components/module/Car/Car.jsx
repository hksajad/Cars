  ...
  return (
    <div className="flex justify-center max-w-[900px] m-auto items-center flex-wrap max-[768px]:max-w-full">
      {carsData.map((indexcar) => (
        <div className="items_cars flex flex-col flex-wrap " key={indexcar.id}>
          {/* img cars */}
          <div className="img_cars max-[768px]:m-2  m-5">
            <Link href={`/AllCars/${indexcar.id}`}>
              <Image
                src={indexcar.image}
                width={300}
                height={300}
                className="w-[250px] h-[250px] rounded-lg  max-[768px]:w-full max-[768px]:m-auto"
                title="Details"
              />
            </Link>
          </div>
          ...
          <div className="location flex justify-around items-center mt-5 mb-5">
            <h1 className="bg-green-200 rounded-lg px-1 py-1">
              {indexcar.price}
            </h1>
